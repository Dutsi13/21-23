...
from aiogram.types import LabeledPrice, PreCheckoutQuery, Message, InputMediaPhoto
...
# ССЫЛКИ НА КАРТИНКИ (Замени на свои FileID или URL)
IMG_MAIN = "https://i.imgur.com/8n9n8n8.jpg"
IMG_CATALOG = "https://i.imgur.com/8n9n8n8.jpg"
IMG_BALANCE = "https://i.imgur.com/8n9n8n8.jpg"
IMG_MY_RENT = "https://i.imgur.com/8n9n8n8.jpg"
...
def back_to_main_kb():
    ...
    kb.button(text="⬅️ Назад в меню", callback_data="to_main")
    return kb
...
async def adm_delacc(message: Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID: return
    if command.args:
        cur.execute('DELETE FROM accounts WHERE phone = ?', (command.args.strip(),))
        db.commit();
...
async def adm_delbal(message: Message, command: CommandObject):
    if message.from_user.id != ADMIN_ID: return
    if command.args:
        cur.execute('UPDATE users SET balance = 0 WHERE user_id = ?', (int(command.args),))
        db.commit();
        await message.answer("Баланс обнулен.")
...
async def adm_setprice(message: Message, command: CommandObject):
    ...
    try:
        ...
        db.commit();
        await message.answer(f"Цена для {phone} теперь ${price}/мин")
    except:
        pass
...
async def adm_give(message: Message, command: CommandObject):
    ...
    try:
        ...
        add_payment_history(int(uid), float(amt), "Admin")
        await message.answer(f"Зачислено ${amt} юзеру {uid}")
    except:
        pass
...
async def bal_menu(message: Message):
    ...
    await message.answer_photo(photo=IMG_BALANCE, caption=f"💳 Ваш баланс: **${bal}**",
                               reply_markup=kb.adjust(1, 2).as_markup())
...
async def catalog(message: Message):
    ...
    await message.answer_photo(photo=IMG_CATALOG, caption="🛒 Свободные номера:", reply_markup=kb.adjust(1).as_markup())
...
async def rent_init(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Минуты аренды (30 - 600):")
...
async def my_rents(event: Message | types.CallbackQuery):
    ...
    if isinstance(event, Message):
        await event.answer_photo(photo=IMG_MY_RENT, caption=cap, reply_markup=kb.adjust(1).as_markup())
    else:
            ...
            for chat in chats:
                ...
                try:
                    if res[5]:  # С фото
                        ...
                        buf.name = "img.jpg"
                        await client.send_file(chat, buf, caption=res[1], force_document=False)
                    else:  # Текст
...
async def set_param_init(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f"Введите новое значение для {param}:")
